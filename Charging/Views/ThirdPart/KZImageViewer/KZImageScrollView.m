...
//        [self zoomToRect:CGRectMake(touchPoint.x - xsize/2, touchPoint.y - ysize/2, xsize, ysize) animated:YES];
        ...
        CGFloat newZoomScale = ((self.maximumZoomScale + self.minimumZoomScale) / 2);
        CGRect zoomRect = [self zoomRectForScale:newZoomScale
...
- (CGRect)zoomRectForScale:(float)scale withCenter:(CGPoint)center {
    ...
    zoomRect.size.height = [_photoImageView frame].size.height / scale;
    zoomRect.size.width  = [_photoImageView frame].size.width  / scale;
    
    center = [_photoImageView convertPoint:center fromView:self];
...
{
    NSLog(@"Content : [%f,%f],[%f,%f]", self.contentOffset.x, self.contentOffset.y, self.contentSize.width, self.contentSize.height);
}
