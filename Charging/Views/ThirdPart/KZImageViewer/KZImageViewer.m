...
- (void)setup
{
    ...
    self.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:1]; // fix #39793
...
//    CGRect frame = CGRectMake((fullWidth - W) / 2, (fullHeight - H) / 2, W, H);
    CGRect frame = CGRectMake(MAX(fullWidth - zoomImageView.contentSize.width,0) / 2 - zoomImageView.contentOffset.x,
                              MAX(fullHeight - zoomImageView.contentSize.height, 0) / 2 - zoomImageView.contentOffset.y,
                              zoomImageView.contentSize.width,
                              zoomImageView.contentSize.height);
    
    //用于动画的ImageView
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:frame];
    imageView.contentMode = zoomImageView.kzImage.srcImageView.contentMode;
    imageView.clipsToBounds = YES;
